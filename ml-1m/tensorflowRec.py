...
df=pd.read_csv('ratings.dat',sep='\s+',names=['user','item','rating','timestamp'])
...
MNS=MinMaxScaler()
rating_scaler=MNS.fit_transform(df.rating.values.reshape(-1,1))
rating_scaler=pd.DataFrame(rating_scaler)
df['rating']=rating_scaler
...
matrix=df_matrix.as_matrix()
users=df_matrix.index.tolist()
items=df_matrix.columns.tolist()
